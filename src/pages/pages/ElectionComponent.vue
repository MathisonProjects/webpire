<template>
    <div>
        ...
        <div class='row'>
            <div class='col-xs-12 col-sm-6 col-md-6' v-for='(item,id) in candidates' :key='id'>
                <v-card class='mt-3'>
                    <v-card-title>
                        <b>{{ item.content.name }}</b>,&nbsp;<i>{{ item.content.description }}</i>
                        <v-spacer />
                        <b>Running For:</b>&nbsp;{{ item.content.running_for }}
                    </v-card-title>
                    <v-card-text>
                        <div class='row'>
